...
const EditCourseModal = ({ course, departments, faculty, onClose, onSubmit }) => {
  ...
  const [newClo, setNewClo] = useState({ 
    description: '', 
    number: (formData.clos && formData.clos.length > 0) ? formData.clos.length + 1 : 1 
  });
  
  // Update newClo number when clos array changes
  useEffect(() => {
    setNewClo(prev => ({
      ...prev,
      number: (formData.clos && formData.clos.length > 0) ? formData.clos.length + 1 : 1
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg w-full max-w-lg mx-4 p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-darktext">Edit Course</h2>
          <button 
            onClick={onClose}
