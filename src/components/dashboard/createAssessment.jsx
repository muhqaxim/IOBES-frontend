...
const CreateAssessment = ({ AssessmentType, onGenerate, onClose }) => {
  ...
  const toggleCLOSelection = (clo) => {
    ...
    const cloId = clo.id; // get CLO ObjectId
    const cloText = clo.description;

    const exists = selected.find((item) => item.id === cloId);

    if (exists) {
      setFormData({
        ...formData,
        selectedCLOs: selected.filter((item) => item.id !== cloId),
      });
    } else {
      setFormData({
        ...formData,
        selectedCLOs: [...selected, { id: cloId, text: cloText }],
      });
    }
  };
  ...
  const generateAssessment = async (e) => {
    e.preventDefault();
    setLoading(true);
    const selectedCLOsText = formData.selectedCLOs
      .map((clo) => clo.text)
      .join("\n");
    ...
    try {
      ...
      console.log("Before Generating:", formData);
      onGenerate(
        generatedAssessment,
        formData.courseId,
        formData.selectedCLOs.map((clo) => clo.id)
      );
    } catch (error) {
        ...
        <form onSubmit={generateAssessment} className="space-y-6">
          ...
          <div className="border p-4 rounded-lg bg-gray-50">
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {formData.selectedCLOs.map((clo, idx) => (
                <div
                  key={clo.id}
                  className="flex items-center bg-blue-100 text-blue-800 px-3 py-1 rounded-full"
                >
                  <span className="mr-2 text-sm">{clo.text}</span>
                  <button
                    type="button"
                    onClick={() => toggleCLOSelection(clo)}
                    ...
                    {clos.map((clo, index) => {
                      ...
                      return (
                        <label key={index} className="block">
                          <input
                            type="checkbox"
                            checked={formData.selectedCLOs.some(
                              (item) => item.id === clo.id
                            )}
